...
__global__ void upsweep(int N, int two_d, int* output) {
  ...
  const int i = index * two_dplus1;
  if (i < N) {
    output[i + two_dplus1 - 1] += output[i + two_d - 1];
  }
}

__global__ void set_to_zero(int N, int* output) {
  output[N - 1] = 0;
}
...
void exclusive_scan(int* input, int N, int* result) {
  ...
  for (int two_d = 1; two_d <= N / 2; two_d *= 2) {
    const int two_dplus1 = 2 * two_d;
    const int nthreads = N / two_dplus1;
    const int nblocks = (nthreads + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
    const int threads_per_block = nblocks == 1 ? nthreads : THREADS_PER_BLOCK;
    upsweep<<<nblocks, threads_per_block>>>(N, two_d, result);
    cudaDeviceSynchronize();
  }

  set_to_zero<<<1, 1>>>(N, result);
  cudaDeviceSynchronize();
  ...
  for (int two_d = N / 2; two_d >= 1; two_d /= 2) {
    ...
    const int nblocks = (nthreads + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
    const int threads_per_block = nblocks == 1 ? nthreads : THREADS_PER_BLOCK;
    downsweep<<<nblocks, threads_per_block>>>(N, two_d, result);
    cudaDeviceSynchronize();
  }
}
...
__global__ void find_repeat_flags(int N, int* device_input, int* flags,
                                  int* out_idxs, int* repeat_idxs) {
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index < N - 1) {  // Skip the last element.
    int flag_i = 0;
    int idx_i = 0;
    ...
    repeat_idxs[index] = idx_i;
  }
}

// Finds repeated elements and adds them to the output array.
__global__ void find_repeat_elements(int N, int* repeat_idxs, int* flags,
                                     int* out_idxs,
                                     // int* num_repeats,
                                     int* device_output) {
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index < N - 1 && flags[index] == 1) {  // Skip the last element.
    const int input_idx = repeat_idxs[index];
    const int out_idx = out_idxs[index];
    device_output[out_idx] = input_idx;
  }
}
...
int find_repeats(int* device_input, int length, int* device_output) {
  ...
  int* flags = nullptr;
  int* repeat_idxs = nullptr;
  int* out_idxs = nullptr;
  cudaMalloc((void **)&flags, N * sizeof(int));
  cudaMalloc((void **)&repeat_idxs, N * sizeof(int));
  cudaMalloc((void **)&out_idxs, N * sizeof(int));
  find_repeat_flags<<<blocks, THREADS_PER_BLOCK>>>(length, device_input, flags,
                                                   out_idxs, repeat_idxs);
  cudaDeviceSynchronize();

  exclusive_scan(flags, length, out_idxs);
  cudaDeviceSynchronize();

  find_repeat_elements<<<blocks, THREADS_PER_BLOCK>>>(length, repeat_idxs,
                                                      flags, out_idxs,
                                                      device_output);
  int* num_ret = new int[1];
  cudaMemcpy(num_ret, out_idxs + N - 1, 1 * sizeof(int), cudaMemcpyDeviceToHost);
